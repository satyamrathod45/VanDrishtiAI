...
export default function Overview() {
  ...
  // Handle batch ingestion completion & update dashboard stats
  const handleBatchComplete = ({ totalImages, tigersFound, blanksQuarantined, crops }) => {
    setOverview((prev) => {
      if (!prev) return prev;
      const currentStats = prev.statistics || {};
      return {
        ...prev,
        statistics: {
          ...currentStats,
          todaysSightings: Number(currentStats.todaysSightings || 0) + (tigersFound || 0),
          totalSightings: Number(currentStats.totalSightings || 0) + (tigersFound || 0),
          pendingReviews: Number(currentStats.pendingReviews || 0) + (crops?.length || 0),
          pendingProcessingImages: Math.max(0, Number(currentStats.pendingProcessingImages || 0) - (totalImages || 0)),
        },
      };
    });
  };
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f7f7f5] px-4 pb-32 pt-6 text-[#202020] sm:px-6 lg:px-8">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1.5">
              ...
              <p className="max-w-[620px] text-[10px] text-white/60 leading-relaxed">
                Ingest SD card images from field camera traps to filter blanks, extract tiger sightings, and prepare high-resolution tiger crops.
              </p>
            </div>
      ...
      {/* ================================================== */}
      {/* FOLDER UPLOAD & BATCH PROCESSING PIPELINE MODAL */}
      {/* ================================================== */}
      {showDetectionModal && (
        <FolderUploadModal
